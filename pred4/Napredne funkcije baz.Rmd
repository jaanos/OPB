...
- Vedno obstaja na bazi shema `public`. 
...
```{sql  eval=FALSE}
CREATE USER janez WITH ENCRYPTED PASSWORD 'triglav';
```
...
```{sql  eval=FALSE}
CREATE USER metka WITH ENCRYPTED PASSWORD 'triglav';
...
BEGIN 
  UPDATE blagajna SET znesek=(SELECT znesek FROM blagajna WHERE ime='Janez') - 5 WHERE ime='Janez';
  UPDATE blagajna SET znesek=(SELECT znesek FROM blagajna WHERE ime='Metka') + 5 WHERE ime='Metka';
COMMIT;
```
